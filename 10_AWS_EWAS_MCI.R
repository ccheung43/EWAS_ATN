...
source("Code/runsvy_stratified.R")
...
MCI_status = c(1,0)
for(subgroup in unique(MCI_status)) { 
  subgroup_condition = expr(MCI == !!subgroup)
  res <- future_map(subchunks, ~ runsvy_stratified(.x, ppath, mpath, biomarkers, subgroup_condition), 
  ...
  lapply(names(res[[1]]), function(biomarker) { 
    ...
    path = "Results/MCI_stratified/"
    file_name = paste0(path, biomarker, "/", biomarker, "_EWAS_MCI", subgroup, "_", blk, ".csv")
